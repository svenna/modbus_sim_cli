...
def path(name=""):
    ...
    if os.path.exists(name):
        return name
    if name.startswith("~"):
        name = name[1:] if os.path.sep == name[2] else name[2:]
        home_dir = os.path.expanduser("~")
        name = os.path.join(home_dir, name)

    if name.startswith(os.path.sep):
        return name
    actual = os.path.dirname(os.path.abspath(__file__)).split(os.path.sep)[:-4]
    if name:
...
def make_dir(name, log=None, msg="", done=">> DONE >> "):
    """
    ...
    if not os.path.exists(name):
        ...
        os.makedirs(name)
        if log:
            log.info("%s%s" % (done, msg))


def remove_file(name, log=None, msg="", done=">> DONE >> "):
    """
    rm <name>
    """
    if os.path.exists(name):
...
def remove_dir(name, log=None, msg="", done=">> DONE >> "):
    """
