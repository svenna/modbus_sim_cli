...
class ModbusSimu(object):
    ...
    def start(self):
        logger.info("Starting Modbus %s server on %s" % (self.server_type, self.port))
        self.server.start()
        if self.server_type == "tcp":
            self._server_add = self.server._sa

    def stop(self):
        logger.info("Stopping Modbus %s server" % self.server_type)
        self.server.stop()
    ...
    def get_slaves(self):
        logger.info("Getting slaves on Modbus %s server" % self.server_type)
...
#
#     if getattr(cfg, "no_modbus_log", False):
